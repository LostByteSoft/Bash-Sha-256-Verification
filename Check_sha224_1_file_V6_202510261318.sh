...
#!/usr/bin/ffmpeg
	...
	version=2025-10-26_Sunday_13:18:19
	hashtype=sha224
...
echo -------------------------========================-------------------------
echo "The core/code program. Check ${hashtype}sum. Version 6.360"
	echo
...
## -f file , -n variable , d folder
	...
	if [ -f "$exist" ]; then
		...
		--file-filter="Hash files (SUM) | *sum* *suM* *sUm* *sUM* *Sum* *SuM* *SUm* *SUM*" \
...
part=$((part+1))
echo "-------------------------===== Section $part =====-------------------------"
echo "Software lead out. Version 7.29"
	echo
