...
echo -------------------------========================-------------------------
echo "The core/code program. Version 6.323"
	echo
...
## -------------------------===== Separator =====-------------------------
	...
	if [ -f "$name1.${hashtype}sum" ]; then
		...
		echo
		echo " --- Possibles match : ---"
		echo
		#find . -type f \( -name "*.crc*" -o -name "*.md5*" -o -name "*.sha*" \)	## all types could causes errors
		find . -type f -name "*.${hashtype}sum"						## only selected hashtype
		echo
		echo " --- ---"
		echo
...
if [ $count -eq "1" ]; then
		...
		if [[ -f "file2" && ! -s "file2" ]]; then
			if [ "$debug" -eq "1" ]; then
				echo "'file2' is empty. Setting variable to 0."
				fi
			file2=0
			logs=0
		elif
		[[ -z "file2" ]]; then
			if [ "$debug" -eq "1" ]; then
				echo "'file2' is empty or unset. Setting variable to 0."
				fi
			file2=0
			logs=0
		else
			if [ "$debug" -eq "1" ]; then
				echo "'$file2' exists and is not empty, or the variable 'file2' contains a non-empty string that is not an empty file."
				echo logs = $logs
				fi
			if [ "$count" -ge "1" ]; then
