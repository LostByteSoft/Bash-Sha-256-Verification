...
#!/usr/bin/ffmpeg
		...
		printf '\033[8;20;100t'		## minimum of 88 for graphics.
...
echo -------------------------========================-------------------------
	...
	echo 2025-05-03_Saturday_09:01:04
echo -------------------------========================-------------------------
...
echo -------------------------========================-------------------------
echo "Functions informations. Version 1,11"
	echo
...
echo -------------------------========================-------------------------
echo "The core/code program. Version 3.108"
	echo
...
## -------------------------===== Separator =====-------------------------
	...
	for i in "$file"/*.*;
		do name=`echo "$i" `
			...
			echo name="$name"
			export VAR="$i"
			sha256sum "$name" | awk '{print $1}' > "/dev/shm/${hashtype}sum.${hashtype}sum"
			read -r firstline<"/dev/shm/${hashtype}sum.${hashtype}sum"
			echo "$firstline  $(basename "${VAR}")" >> ""$file"/${hashtype}sum.${hashtype}sum"
