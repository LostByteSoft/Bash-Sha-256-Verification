...
if test -f "$exist"; then
	...
	hashtypefile="$(zenity --file-selection --filename=$HOME/$USER --file-filter="*.${hashtype}sum *.txt" --title="OPTIONAL - Associated ${hashtype}sum file NOT exist, must select OR press CANCEL to create a file *.${hashtype}sum *.txt")"
