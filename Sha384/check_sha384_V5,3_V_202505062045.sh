...
#!/usr/bin/ffmpeg
	...
	version=2025-05-06_Tuesday_20:45:24
...
echo -------------------------========================-------------------------
echo "The core/code program. Check ${hashtype}sum. Version 5.328"
	echo
	debugcore=0
	readdata=0
	calsum1=
...
## -------------------------===== Separator =====-------------------------
	...
	if [ "$debugcore" -eq "0" ]; then
		...
		printf '\033[8;40;140t'		# will resize the window.
	fi
...
if [ -f "$exist" ]; then
	...
	echo "${green}	███████████████████████████████████████████████████████████████${reset}"
	echo
	echo "	OPTIONAL - Associated ${hashtype}sum file NOT FOUND, must select a file containing "
	echo "	${hashtype}sum OR ${hashtype2}SUMS OR press CANCEL to create a file ${hashtype}sum"
	echo
	echo "${green}	███████████████████████████████████████████████████████████████${reset}"
	echo
	echo "${yellow}	███████████████████████████████████████████████████████████████${reset}"
	...
	#hashtypefile="$(zenity --file-selection --filename=$HOME/$USER --file-filter="*.${hashtype}sum *.${hashtype2}SUMS *.txt" \
	#--title="OPTIONAL - Associated ${hashtype}sum ${hashtype2}SUM file NOT found, must select OR press CANCEL to create a file *.${hashtype}sum *.txt")"
...
part=$((part+1))
echo "-------------------------===== Section $part =====-------------------------"
echo "!!! WAIT !!! Calculations started. If you press cancel on load you will create a new hash file."
	echo
	echo "	${green}████████████████████████████████████████████${reset}"
	echo "	${green}██                                        ██${reset}"
	echo "	${green}██        WAIT hash calculating...        ██${reset}"
