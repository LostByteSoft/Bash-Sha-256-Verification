...
echo -------------------------========================-------------------------
echo "The core/code program. Version 6.002"
	echo
	debugcore=0
	found=0
...
echo "Check all files in folder with an $hashtype.sum"
	...
	if [ -f "$name1.${hashtype}sum" ]; then
		...
		echo "Select ${hashtype}sum using dialog !"
			...
			file2="$(zenity --file-selection --file-filter="*.${hashtype} *.${hashtype}sum *.txt *.hash" --filename=$file/ --title="Select a ${hashtype} or hash format supported")"
	...
	if [ "$debugcore" -eq "1" ]; then
		...
		echo name = $name
		echo
		echo echo "$file"/${hashtype}sum -c "$file2"
		echo
		fi
