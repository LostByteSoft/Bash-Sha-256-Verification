...
echo -------------------------========================-------------------------
echo "The core/code program. Version 6.315"
	echo
...
## -------------------------===== Separator =====-------------------------
	...
	if [ -f "$name1.${hashtype}sum" ]; then
		found=1
		working=1
		echo "${green}█████${reset} File found : $name1.${hashtype}sum"
		echo
	else
		found=0
		working=0
		echo "${yellow}█████${reset} File NOT found : $name1.${hashtype}sum"
		echo
		echo "Select ${hashtype}sum using dialog !"
		echo
			
			file2="$(zenity --file-selection --file-filter="*.${hashtype}sum" --filename=$file/ --title="Select a ${hashtype} or hash format supported")"

			count=`ls -1 "$file2" 2>/dev/null | wc -l`
			if [ $count -ge "1" ]; then
				working=1
				echo "${green}█████${reset} File selected : $file2"
				echo
			else
				found=2
				working=0
	...
	if [ "$found" -eq "1" ]; then
		...
		echo
			if [ "$logs" -eq "1" ]; then
				...
				touch ""$(basename "${VAR}")_SUCESS_LOG_${now6}.${hashtype}
				${hashtype}sum -c "$file/$name1.${hashtype}sum" | tee "$(basename "${VAR}")"_SUCESS_LOG_${now6}.${hashtype}
			else
				${hashtype}sum -c "$file/$name1.${hashtype}sum"
			fi
		error
		...
		if [ "$logs" -eq "1" ]; then
			echo "Log created : "$(basename "${VAR}")_SUCESS_LOG_${now6}.${hashtype}
			echo
			fi
	...
	if [ "$found" -eq "0" ]; then
		...
		cd "$name"
			if [ "$logs" -eq "1" ]; then
				now6=$(date +"%Y-%m-%d_%H:%M")
				touch ""$(basename "${VAR}")_SUCESS_LOG_${now6}.${hashtype}
				${hashtype}sum -c "$file2" | tee ""$(basename "${VAR}")_SUCESS_LOG_${now6}.${hashtype}
			else
				${hashtype}sum -c "$file2"
			fi
		error
		if [ "$primeerror" -ge "1" ]; then
			echo
