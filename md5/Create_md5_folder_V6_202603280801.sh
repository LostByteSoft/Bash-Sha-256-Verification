...
#!/usr/bin/ffmpeg
	...
	version=2026-03-28_Saturday_08:01:30
	hashtype=md5
...
echo -------------------------========================-------------------------
echo "The core/code program. Version 6.366"
	echo
...
## -------------------------===== Separator =====-------------------------
	...
	if [ "$debugcore" -eq "0" ]; then
		## normal operation
		printf '\033[8;20;125t'		# will resize the window.
	else
		## debugcore
...
## Read and remove line
	...
	functionremovehashname () {
		...
		grep -v "md5sum" "/dev/shm/$random.txt" > "/dev/shm/$random.txt0"
		grep -v "sha224sum" "/dev/shm/$random.txt0" > "/dev/shm/$random.txt1"
		...
		grep -v "sha512" "/dev/shm/$random.txt11" > "/dev/shm/$random.txt15"
		## remove folder name
	...
	if [ "$findsubfolders" -gt 0 ]; then
		...
		echo --- Listing end ---
			...
			if zenity --question --text="YES i want the subfolder(s).\n\nNO i don't want findsubfolders." --title="Directory save."; then
					...
					find . > "/dev/shm/$random.txt"			## all files and subdirectory
				else
					echo
