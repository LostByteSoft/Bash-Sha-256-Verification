...
echo -----------------------------------------------------------------------------
echo "Sha-1-sum verification, you will select 1 or 2 files"
echo "Version 4"
...
exist="$file".sha1
	...
	echo "An associated sha1 files exist, autoread"
	#read -r firstline<"$file.sha1"
	sha1file="$file".sha1

else
	echo "OPTIONAL - Associated sha1 file NOT exist, must select OR create (*.sha1 *.txt)"
sha1file="$(zenity --file-selection --filename=$HOME/$USER --file-filter="*.sha1 *.txt" --title="OPTIONAL - Associated sha1 file NOT exist, must select OR create (*.sha1 *.txt)")"
...
if test -z "$sha1file"
	then
		echo "You don't have selected a file. You will create a new *.sha1 file."
		echo Press ENTER to continue the creation of a .sha1 file OR press X to quit.
		read name
			cd /"$(dirname "${VAR1}")"
			echo -----------------------------------------------------------------------------
			...
			echo -----------------------------------------------------------------------------
			read -r firstline2<"${file}.sha1"
			export VAR3="$file"
			#echo "$(dirname "${VAR3}")"
			#echo "$(basename "${VAR3}")"
			#echo var is a var : "$(dirname "${VAR3}")"
			echo "$firstline2  $(basename "${VAR3}")" > "${file}.sha1"
	...
	echo "An sha1 file is selected and continue (no creation)."
fi
}
...
echo -----------------------------------------------------------------------------
	...
	firstline="`grep "$(basename "${VAR2}")" "$sha1file"`"

	echo Debug echo, you need 3 variable here.
	echo Var 1 file = "$file"
	echo Var 2 sha1file = "$sha1file"
	...
	#echo dirname = "$(dirname "${VAR2}")"
	#echo basename = "$(basename "${VAR2}")"
	sha1sum "$file" | awk '{print $1}' > "/dev/shm/"$(basename "${VAR2}")".sha1"
	read -r calsum<"/dev/shm/"$(basename "${VAR2}")".sha1"
	echo "Your selected file for hash is : "
	echo $file
	echo "The ckeck sum calculated is :"
	echo calsum = "$calsum"
	echo "$calsum $(basename "${VAR2}")" > "/dev/shm/"$(basename  "${VAR2}")".sha1"
	echo ------------------------
	echo "Your selected sha1 file is : (Only if you have selected one) "
	echo "$sha1file"
	echo -----------------------------------------------------------------------------
	...
	echo "${green}sha1sum with compare hash is EQUAL${reset}"
	#echo "sha1sum with compare hash is EQUAL"
	else
	
	echo "${red}sha1sum with compare hash ARE NOT equal${reset}"
	#echo "sha1sum with compare hash ARE NOT equal"
	fi
