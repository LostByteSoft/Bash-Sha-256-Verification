...
## Software name, what is this, version, informations.
	echo "Software name: check md5 Manual Input"
	echo "File name : check_md5_Manual.sh"
	echo
	echo "What it does ? Compare the calculated md5 of a file with an md5 you specified."
	echo
...
echo -------------------------========================-------------------------
	...
	entry=$(zenity --entry --width 550 --title "Check md5" --text "Copy paste md5 here. ( NOT case sensitive. )")

if test -z "$entry"
	then
		echo "You don't have entered an md5, now exit in 3 seconds."
		echo -------------------------========================-------------------------
		sleep 3
		exit
	else
...
## The code program.
	...
	rm "/dev/shm/temp.md5" 2> /dev/null

	entry="$(echo $entry | tr '[A-Z]' '[a-z]')"
	#echo "md5sum "$file" | awk '{print $1}'"
	echo
	echo WAIT calculating...
	md5sum "$file" | awk '{print $1}' > "/dev/shm/temp.md5"
	read -r calsum < "/dev/shm/temp.md5"
...
if [ "$calsum" = "$entry" ]; then
	...
	echo "${green}█████████████████████████████████████████${reset}"
		...
		if zenity --question --no-wrap --text="Do you want to create a md5 file associed ?\n\nYES = create file : ""$(basename "${VAR}")"".md5"
			then 
			echo The ckeck sum calculated is :
			#md5sum "$file" | awk '{print $1}' > "${file}.md5"
			#read -r firstline2<"${file}.md5"
			#echo "$firstline2  $(basename "${VAR}")" > "${file}.md5"
			read -r calsum1 < "/dev/shm/temp.md5"
			echo Is writen : "$calsum1  "$(basename "${VAR}")""
			echo To file: "${file}.md5"
			echo " $calsum1  "$(basename "${VAR}")"" > "${file}.md5"
			fi
