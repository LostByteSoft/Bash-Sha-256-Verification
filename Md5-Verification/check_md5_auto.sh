...
## Software name, what is this, version, informations.
	...
	echo
	echo What it does ?
	echo "md5-sum verification, you will select 1 or 2 files"
	echo
	echo Informations :
	...
	echo "Must by have 2 space between name and sha in *.md5 file"
...
## search for file with the same name
	...
	echo "An associated md5 files exist, autoread"
	#read -r firstline<"$file.md5"
	md5file="$file".md5
	fi

## check for a file md5.md5
cd "$(dirname "${VAR}")"
	exist="md5.md5"
	if test -f "$exist"; then
	echo "An associated md5 files exist, autoread"
...
#echo "OPTIONAL - Associated md5 file NOT exist, must select OR create (*.md5 *.txt)"
#md5file="$(zenity --file-selection --filename=$HOME/$USER --file-filter="*.md5 *.txt" --title="OPTIONAL - Associated md5 file NOT exist, must select OR create (*.md5 *.txt)")"



	#else
	#	echo "An md5 file is NEEDED to continue."
	#fi
...
#put the good line in the variable firstline
	...
	## firstline=`grep "$(basename "${VAR2}")" "$md5file"`
	##fgrep -F "$(basename "${VAR2}")" "$md5file" >> firstline
	## grep string filename
	## grep example document.txt
	firstline=$(grep "$(basename "${VAR2}")" $md5file)
	echo Debug echo, you need 3 variable here.
	echo Var 1 file = "$file"
	echo Var 2 md5file = "$md5file"
...
#two file have ben provided.
	...
	#echo "/dev/shm/"$(basename "${VAR2}")".md5"

	md5sum "$file" | awk '{print $1}' > "/dev/shm/"$(basename "${VAR2}")".md5"
	read -r calsum<"/dev/shm/"$(basename "${VAR2}")".md5"
	echo "Your selected file for hash is : "
	echo $file
	echo "The ckeck sum calculated is :"
	echo calsum = "$calsum"
	echo "$calsum $(basename "${VAR2}")" > "/dev/shm/"$(basename  "${VAR2}")".md5"
	echo "Your selected or auto-load md5 file is :"
	echo "$md5file"
...
## Verify if 2 variable is equal	
	...
	echo "${green}████████████████████████████████████████${reset}"
	echo "${green}██md5sum with compare hash is EQUAL██${reset}"
	echo "${green}████████████████████████████████████████${reset}"
	#echo "md5sum with compare hash is EQUAL"
	else

	echo "${red}█████████████████████████████████████████████${reset}"	
	echo "${red}██md5sum with compare hash ARE NOT equal██${reset}"
	echo "${red}█████████████████████████████████████████████${reset}"
	#echo "md5sum with compare hash ARE NOT equal"
	fi
