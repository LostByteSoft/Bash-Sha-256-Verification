...
#!/usr/bin/ffmpeg
	...
	version=2025-10-17_Friday_07:44:43
	hashtype=crc32
...
echo -------------------------========================-------------------------
echo "The core/code program. Check crc32. Version 6.353"
	echo
...
if command -v crc32 >/dev/null 2>&1
	...
	echo "		${blue}████████████████████████████████████████████${reset}"
	echo "		${blue}██                                        ██${reset}"
	echo "		${blue}                 crc32        ${reset}"
	echo "		${blue}██                                        ██${reset}"
	echo "		${blue}████████████████████████████████████████████${reset}"
	echo
	else
		echo "${red}████████████████ You don't have ' crc32 ' installed. ████████████████${reset}"
		echo
...
## -------------------------===== Separator =====-------------------------
	...
	#echo "	Searching for file named : $(basename "${VAR}").crc32"
	#echo
...
## -f file , -n variable , d folder
	...
	if [ -f "$exist" ]; then
		echo "	An associated crc32 file exist, autoread."
		hashtypefile=$exist
		echo "	hashtypefile = $hashtypefile"
	else
		echo "${blue}	███████████████████████████████████████████████████████████████${reset}"
		echo
		echo "	Associated $(basename "${VAR}").crc32 NOT found."
		echo
		echo "${blue}	███████████████████████████████████████████████████████████████${reset}"
		echo
		...
		echo "${green}	███████████████████████████████████████████████████████████████${reset}"
		echo
		echo "	OPTIONAL - Select a file including crc32"
		echo "		OR"
		echo "	press CANCEL to create a new crc32 file :"
		echo
		echo "	"$file".crc32"
		echo
		echo "${green}	███████████████████████████████████████████████████████████████${reset}"
		hashtypefile="$(zenity --file-selection --filename=$HOME/$USER --file-filter="*CRC32* *crc32*" \
...
## If you press cancel on load you will create a new hash file
	...
	if test -z "$hashtypefile"; then
		echo "You don't have selected a file. You will create a new .crc32 file."
		newhash=1
		cd /"$(dirname "${VAR1}")"
		echo
		...
		echo
		crc32 "$file" | awk '{print $1}' > "${file}.crc32"
		read -r firstline2<"${file}.crc32"
		echo "$firstline2  $(basename "${VAR}")" > "${file}.crc32"
		...
		echo "An crc32 file is selected and continue (no creation)."
		newhash=0
		echo
...
if [ "$bypass" -eq "0" ]; then
	...
	echo "-------------------------===== Section $part =====-------------------------"
		echo
		echo "${blue}	█████████████████ WORKING WAIT !!! crc32 ███████████████████${reset}"
		echo
		calsum2=$(crc32 "$file" | awk '{print $1}')
		echo
		echo "$file"
		echo
...
echo "Hash check."
	...
	if [ "$newhash" -eq "0" ]; then
		if [ "$calsum1" = "$calsum2" ]; then
			echo "	${green}█████████████████████████████████████████████████████████████${reset}"
			echo "	${green}██                                                         ██${reset}"
			echo "	${green}         crc32 with compare hash is EQUAL       ${reset}"
			echo "	${green}██                                                         ██${reset}"
			echo "	${green}█████████████████████████████████████████████████████████████${reset}"
			echo
			sleep=4
			...
			echo "	${red}███████████████████████████████████████████████████████████████████${reset}"
			echo "	${red}██                                                               ██${reset}"
			echo "	${red}         crc32 with compare hash ARE NOT equal       ${reset}"
			echo "	${red}██                                                               ██${reset}"
			echo "	${red}███████████████████████████████████████████████████████████████████${reset}"
			echo
			sleep=8
	...
	if [ "$newhash" -eq "1" ]; then
		echo "	${green}█████████████████████████████████████████████████████████████${reset}"
		echo "	${green}██                                                         ██${reset}"
		echo "	${green}        New hash file with crc32 has been calculated.       ${reset}"
		echo "	${green}██                                                         ██${reset}"
		echo "	${green}█████████████████████████████████████████████████████████████${reset}"
		echo
		sleep=4
