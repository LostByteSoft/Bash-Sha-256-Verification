...
#!/usr/bin/ffmpeg
	...
	echo "-------------------------===== Start of bash ====-------------------------"
	printf '\033[8;27;125t'	## minimum of 90 for graphics.
	version=2025-10-12_Sunday_16:45:46
	hashtype=sha256
...
echo -------------------------========================-------------------------
echo "The core/code program. Version 6.135"
	echo
	debugcore=0
...
if zenity --question --text="YES i want the hash file in the same directory.\n\nNO i want the hash file in the previous directory." --title="Directory save."; then
		...
		printf '\033[8;20;110t'	## minimum of 90 for graphics.
		...
		echo "-------------------------===== Section $part =====-------------------------"
		echo
		echo "This will create file : $file/$name1.${hashtype}sum"
		#cp "/dev/shm/${hashtype}sum.${hashtype}sum" "$file/$name1 ${hashtype}sum.${hashtype}sum"
		...
		echo
		echo "SPACES ARE NOT SUPPORTED IN FOLDER NAMES."
		echo
		...
		echo ""$(dirname "${VAR1}")"/$name1.${hashtype}sum"
		#cp "/dev/shm/${hashtype}sum.${hashtype}sum" ""$(dirname "${VAR1}")"/$name1 ${hashtype}sum.${hashtype}sum"
		cp "/dev/shm/${hashtype}sum.${hashtype}sum" ""$(dirname "${VAR1}")"/$name1.${hashtype}sum"
		error
...
part=$((part+1))
echo "-------------------------===== Section $part =====-------------------------"
echo "Software lead out. Version 7.25"
	echo
...
## Exit, wait or auto-quit.
	...
	if [ "$primeerror" -ge "1" ]; then
		...
		if [ "$reseize" -eq "1" ]; then
			printf '\033[8;21;90t'		## Will resize the window.
			fi
	...
	if [ "$noquit" -eq "1" ]; then
		...
		echo
		while true; do 
			echo "Press ANYKEY or SPACE to show EULA, ENTER to EXIT." 
