...
echo -------------------------========================-------------------------
echo "The core/code program. Version 6.344"
	echo
	debugcore=0
...
## -------------------------===== Separator =====-------------------------
	...
	if [ "$debugcore" -eq "1" ]; then
		printf '\033[8;40;120t'	## minimum of 90 for graphics.
...
echo "-------------------------===== Section $part =====-------------------------"
	...
	if [ "$reseize" -eq "1" ]; then
		printf '\033[8;40;90t'	## minimum of 90 for graphics.
		fi
...
if zenity --question --text="YES i want the subfolder(s).\n\nNO i don't want subfolders." --title="Directory save."; then
		...
		functionremovehashname
		cat "/dev/shm/$random.txt1" | xargs -I {} md5sum "{}" | tee "/dev/shm/${hashtype}sum.${hashtype}sum"
...
## -------------------------===== Separator =====-------------------------
	...
	echo "-------------------------===== Section $part =====-------------------------"
	echo
	echo "File created : $file/$name1.${hashtype}sum"
