...
#!/usr/bin/ffmpeg
	...
	version=2025-11-07_Friday_18:09:19
	hashtype=sha256
...
echo -------------------------========================-------------------------
echo "The core/code program. Check ${hashtype}sum. Version 6.361"
	echo
...
## -f file , -n variable , d folder
	...
	if [ -f "$exist" ]; then
		...
		hashtypefile="$(zenity --file-selection --filename=$HOME/$USER --file-filter="TXT | *.txt *.txt *.Txt *.tXt *.txT *.TXt *.TxT *.tXT *.TXT" \
		--title="OPTIONAL - Associated ${hashtype} hash file NOT found, select one OR press CANCEL to create a new hash ${hashtype}sum file.")"
		fi
