...
#!/usr/bin/ffmpeg
	...
	version=2026-06-02_Tuesday_09:38:00
	hashtype=sha256
...
echo -------------------------========================-------------------------
echo "The core/code program. Version 6.475"
	echo
...
## -------------------------===== Separator =====-------------------------
	...
	if [ "$debugcore" -eq "0" ]; then
		## normal operation
		printf '\033[8;20;160t'		# will resize the window.
	else
		## debugcore operation
		...
		echo
		echo name1 = $name1 noquit = $noquit
		echo
...
## Read and remove line
	...
	functionremovehashname () {
		## -------------------------===== Separator =====-------------------------
		## remove junk
		grep -v "Trash-1000" "/dev/shm/hash${random}0.txt" > "/dev/shm/hash${random}1.txt"
		grep -v '^\.$' "/dev/shm/hash${random}1.txt" > "/dev/shm/hash${random}2.txt"
		## remove selected type
		grep -v "md5sum" "/dev/shm/hash${random}2.txt" > "/dev/shm/hash${random}3.txt"
		grep -v "sha224sum" "/dev/shm/hash${random}3.txt" > "/dev/shm/hash${random}4.txt"
		grep -v "sha256sum" "/dev/shm/hash${random}4.txt" > "/dev/shm/hash${random}5.txt"
		grep -v "sha384sum" "/dev/shm/hash${random}5.txt" > "/dev/shm/hash${random}6.txt"
		grep -v "sha512sum" "/dev/shm/hash${random}6.txt" > "/dev/shm/hash${random}7.txt"
		grep -v "md5" "/dev/shm/hash${random}7.txt" > "/dev/shm/hash${random}8.txt"
		grep -v "sha256" "/dev/shm/hash${random}8.txt" > "/dev/shm/hash${random}9.txt"
		grep -v "sha384" "/dev/shm/hash${random}9.txt" > "/dev/shm/hash${random}10.txt"
		grep -v "sha512" "/dev/shm/hash${random}10.txt" > "/dev/shm/hash${random}11.txt"
		## remove folder
		grep -E '\.[^/]+$' "/dev/shm/hash${random}10.txt" > "/dev/shm/hash${random}20.txt"
		## -------------------------===== Separator =====-------------------------

		if [ "$debugcore" -eq "1" ]; then
			noquit=1
			echo "--- DEBUG : cat /dev/shm/hash${random}20.txt ---"
			echo
			cat "/dev/shm/hash${random}20.txt"
			echo
			echo --- end cat ---
			echo
			echo "DEBUG : "$file/$name1.$now7.${hashtype}sum""
			echo
			echo "DEBUG : "$file/$name1.${hashtype}sum""
			echo
			fi
		}
...
## Remove temp files
	...
	functionerase () {
		...
		rm -f "/dev/shm/hash${random}"*.txt
		echo "${blue}	█████ Temp files erased █████${reset}"
		}
...
part=$((part+1))
echo "-------------------------===== Section $part =====-------------------------"
echo "Select YES or NO. findsubfolders and date to filename."
	echo
	findsubfolders=$(find "$file" -mindepth 1 -maxdepth 1 -type d | wc -l)
	
	echo "Debug data : findsubfolders = $findsubfolders"
	echo
	
	now7=$(date +"%Y-%m-%d_%H:%M:%S")				## time now

	if [ "$findsubfolders" -gt 0 ]; then
	
		#cd "$file" > /dev/null 2>&1
		#cd "$file"

		echo '--- Listing of subfolders ---'
		echo
		#find "$file" -type d | grep -vE '/lost\+found|/.Trash-1000' | sort  > "/dev/shm/findfolders$random.txt"	## errors shown

			## -------------------------===== Separator =====-------------------------
			## Remove all errors and clean up before show folders
			rm "/dev/shm/findfolders0.txt" 2> /dev/null
			rm "/dev/shm/findfolders1.txt" 2> /dev/null
			rm "/dev/shm/findfolders2.txt" 2> /dev/null
			rm "/dev/shm/findfolders3.txt" 2> /dev/null
			find "$file" -type d 2>/dev/null | sort > "/dev/shm/findfolders0.txt"
			grep -v "lost+found" "/dev/shm/findfolders0.txt" > "/dev/shm/findfolders1.txt"
			grep -v "Trash-1000" "/dev/shm/findfolders1.txt" > "/dev/shm/findfolders2.txt"
			grep -v '^\.$' "/dev/shm/findfolders2.txt" > "/dev/shm/findfolders3.txt"
			cat "/dev/shm/findfolders3.txt"
			rm "/dev/shm/findfolders0.txt" 2> /dev/null
			rm "/dev/shm/findfolders1.txt" 2> /dev/null
			rm "/dev/shm/findfolders2.txt" 2> /dev/null
			rm "/dev/shm/findfolders3.txt" 2> /dev/null
		...
		echo '--- Listing end ---'
			...
			if zenity --question --text="YES i want the subfolder(s).\n\nNO i don't want findsubfolders." --title="Directory save."; then

					echo
					echo "Include subfolders..."
					echo
					if zenity --question --title="${hashtype}sum save file." --width=500 --height=150 \
					--text="YES. I want the date added to the file name.\n\n$file/$name1.$now7.${hashtype}sum\n\nNO. I don't want the date.\n\n$file/$name1.${hashtype}sum\n\n\tDefault NO"
					then
						date7=1
					else
						date7=0
					...
					echo
					cd "$file"
					##find . -type f > "/dev/shm/$random.txt"	## juste first subfolder
					find . > "/dev/shm/hash${random}0.txt"		## all files and subdirectory
					#find . -path './lost+found' -prune -o -print > "/dev/shm/hash${random}0.txt"
				else
					echo
					echo "NO subfolder included..."
					echo
					if zenity --question --title="${hashtype}sum save file." --width=500 --height=150 \
					--text="YES. I want the date added to the file name.\n\n$file/$name1.$now7.${hashtype}sum\n\nNO. I don't want the date.\n\n$file/$name1.${hashtype}sum\n\n\tDefault NO"
					then
					...
					echo
					cd "$file"
					find . -maxdepth 1 -type f > "/dev/shm/hash${random}0.txt"
					#find . -path './lost+found' -prune -o -print > "/dev/shm/hash${random}0.txt"
			fi
	else
			echo
...
## -------------------------===== Separator =====-------------------------
	...
	## The magic line is this one where hash are calculated
	cat "/dev/shm/hash${random}20.txt" | xargs -r -d '\n' ${hashtype}sum | tee "/dev/shm/${hashtype}sum.${hashtype}sum"

	echo
	echo ## extra echo here, does not put skip line if not included
	error

	count=$(grep -c "" "/dev/shm/${hashtype}sum.${hashtype}sum")

	## -------------------------===== Separator =====-------------------------
	## Save the file.

		if [ "$date7" -eq "1" ]; then
				cp "/dev/shm/${hashtype}sum.${hashtype}sum" "$file/$name1.$now7.${hashtype}sum"
	...
	if [ "$date7" -eq "1" ]; then
		#echo date7 = $date7
		#echo now7 = $now7
		echo "$file/$name1.$now7.${hashtype}sum"
	else
		echo "$file/$name1.${hashtype}sum"
	fi

	echo
	echo "Numbers of files hashed : $count (You can close this windows, click X)"
	echo
...
## -------------------------===== Separator =====-------------------------
	...
	if [ "$debugcore" -ge "1" ]; then
		...
		echo "basename = "$(basename "${VAR1}")""
		echo
		read -n 1 -s -r -p "DEBUG - Press any key to continue. (Goes to erase temp files)"
		echo
		echo
		fi

	if [ "$primeerror" -ge "1" ]; then
		read -n 1 -s -r -p "Press any key to continue. (Goes to erase temp files)"
		echo
		echo
		fi
