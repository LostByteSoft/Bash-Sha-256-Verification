...
echo -------------------------========================-------------------------
echo "The core/code program. Create hash $hashtype. Version 6.310"
	echo
	...
	if [ $debugcore -eq "1" ]; then
		...
		echo file "${file}.${hashtype}sum"
		echo
		sleep 2
		fi

	if test -f "${file}.${hashtype}sum"; then
		echo "File exist overwrite ?"

		if zenity --question --text="${file}.${hashtype}sum\n\nFile exist overwrite ?\n\nNO quit\n\nYES overwrite" --width=400; then
				overwrite=0
				#echo overwrite $overwrite
				echo
			else
	...
	if [ $overwrite -eq "0" ]; then
		if [ $reseize -eq "1" ]; then
			printf '\033[8;13;105t'		## Will resize the window.
			fi
		echo "Calculations in progress : ${blue}█████████ WAIT █████████${reset}"
		echo
		...
		echo
		echo "The ckeck sum calculated is : $firstline2"
		echo
