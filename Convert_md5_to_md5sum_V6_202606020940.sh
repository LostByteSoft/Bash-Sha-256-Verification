...
#!/usr/bin/ffmpeg
	...
	version=2026-06-02_Tuesday_09:40:10
	hashtype=md5
...
## -------------------------===== Separator =====-------------------------
echo "The core/code program. Version 6.017"
	echo
...
echo "Select a file using dialog !"
	...
	file="$(zenity --file-selection --filename=$HOME/ --file-filter="*.md5 *.md5sum *.TXT" --title="Select a file, ${hashtype} format supported")"	## File select.
...
## -------------------------===== Separator =====-------------------------
	...
	echo "Removing all lines with hash sum :"
	echo
